...
def mpirun(collective, gpus, xml, txt, lower='1440B', upper='3GB'):
    cmd = f'mpirun -np {gpus} -x NCCL_DEBUG=INFO -x NCCL_ALGO=SCCL -x LD_LIBRARY_PATH={home}/msccl/build/lib/ ' \
        f'-x SCCL_XML_FILES={xml} {home}/nccl-tests/build/{collective}_perf ' \
        f'-g 1 -n 1000 -w 1000 -f 2 -c 1 -z 0 -b {lower} -e {upper} > {txt}'
    print(f'Running {cmd}')
    os.system(cmd)

def mpirun_no_channel(collective, gpus, txt, lower='1440B', upper='3GB', algo='RING,TREE'):
...
if __name__ == '__main__':
    ...
    parser.add_argument('machine', type=str, choices=['dgx2', 'ndv4'], help ='What machine type')
