...
def allreduce_allpairs(gpus, instances, protocol):
    ...
    with MSCCLProgram("allreduce_pairs", topology, collective, instances, protocol=protocol, 
        ...
        for r in range(size):
            ...
            if r == 7:
                c = c.copy(7-2, 'scratch2', index=0, sendtb=(7-2), recvtb=7)
                c = c.copy(r+size, 'scratch2', index=0, sendtb=size+1, recvtb=size+1, ch=r%2)
            else:
                c = c.copy(r+size, 'scratch2', index=0, sendtb=size, recvtb=size+1, ch=r%2)

            c2 = chunk(r+size, Buffer.input, index, size)
            if r == 7:
                c2 = c2.copy(7-2, 'scratch2', index=0, sendtb=size+1, recvtb=size+1, ch=r%2)
                c2 = c2.copy(7, 'scratch2', index=0, sendtb=7, recvtb=(7-2))
            else:
